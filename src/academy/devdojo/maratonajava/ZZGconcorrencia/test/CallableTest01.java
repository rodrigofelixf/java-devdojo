...
class RandomNumberCallable implements Callable<String> {
    ...
    public String call() throws Exception {
        ...
        for (int i = 0; i < num; i++) {
            System.out.printf("%s Running a callable task...%n", Thread.currentThread().getName());
        }
        return String.format("%s finiseh and the random number is %d", Thread.currentThread().getName(), num);
    }
}
